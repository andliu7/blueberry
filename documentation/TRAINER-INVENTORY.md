...
This is the complete list of every place in `src/game/` where a student manipulates or answers a
chemistry question interactively, every route that reaches one, every grading path behind one, and
every reference asset a critic is meant to compare one against. It covers `src/game/`, `packages/`
...
citations drifted furthest during the audit itself were `src/game/lesson/ProblemView.tsx` and
`src/game/lesson/LessonPlayer.tsx`, both re-checked and re-written in instance 11. If a
...
Two facts that will trip you if you do not read them first. There is no `apps/` directory here at
all: this inventory was written against `blueberry_game`, where the game lived at `apps/web/src/`,
and the game moved to `grignard-app-source/src/game/` on 2026-09-08. Every path below was rewritten
to the new root on 2026-09-22 and spot-checked on disk. `packages/economy` **does exist**, and
`CLAUDE.md`'s repository-layout table omits it.
...
|---|---|---|---|---|---|---|---|---|---|---|
| 1 | Trainer, reaction mode (TRUE ARROW-PUSHING) | `src/game/tabs/trainer/TrainerTab.tsx`, mounting `tabs/trainer/engine/TrainerScreen.tsx` | `#/trainer` plus `?reaction=<id>#/trainer` | Tab bar, pathway node, in-tab picker | Single-step mechanism | Yes, the engine's UNDO chip | Yes, the engine's scrubber | Yes, the engine's REDRAW chip, absent on the won screen | Yes, constant width | `gradeDrawing`, `src/game/tabs/trainer/grade.ts:72` |
| 2 | Trainer, sequence mode (TRUE ARROW-PUSHING, arrowless glyphs) | `src/game/tabs/trainer/TrainerTab.tsx`, mounting `tabs/trainer/engine/TrainerScreen.tsx` | `?sequence=<id>#/trainer` | Pathway node, in-tab picker | Multi-step mechanism | Yes, the engine's UNDO chip | Yes, the engine's scrubber; CONTINUE advances the step | Yes, the engine's REDRAW chip, absent on the won screen | No, electron dots | `gradeDrawing`, `src/game/tabs/trainer/grade.ts:72` |
| 3 | Trainer, resonance hunt (TRUE ARROW-PUSHING, arrows forced) | `src/game/tabs/trainer/TrainerTab.tsx`, mounting `tabs/trainer/engine/TrainerScreen.tsx` | `?hunt=<id>#/trainer` | Pathway node, in-tab picker | Resonance, one species | Yes, the engine's UNDO chip | Yes, the engine's scrubber | Yes, the engine's REDRAW chip, absent on the won screen | Yes, forced on | `gradeDrawing`, `src/game/tabs/trainer/grade.ts:72` |
| 5 | MCQ beat (ADJACENT SURFACE) | `src/game/beats/mcq/McqRunner.tsx` | `#/lesson/<node>` | Pathway START via ChargeGate, trainer picker | Pick one, then Check | No, pick is free until Check | No | No | No | `gradeMcq`, `src/game/beats/mcq/grade.ts:117` |
| 6 | Match board beat (ADJACENT SURFACE) | `src/game/beats/match/MatchBoard.tsx` | `#/lesson/<node>` | Same as row 5 | Pair prompt to target | No | No | No, wrong pairs bounce | No | `beatResultFor`, `src/game/beats/match/board.ts:402` |
| 7 | Sort ladder beat (ADJACENT SURFACE) | `src/game/beats/sort/SortBeatView.tsx` | `#/lesson/<node>` | Same as row 5 | Rank cards on a ladder | No | No | Yes, Adjust the ladder | No | `judgeSort`, `src/game/beats/sort/judge.ts:127` |
| 8 | Synthesis gap beat (ADJACENT SURFACE) | `src/game/beats/synthesis/SynthesisGapBeat.tsx` | `#/lesson/<node>` | Same as row 5 | Fill the blank in a route | Clear the blank, but only when picking from the bank | No | Yes, before Check | No | `gradeSynthesisGap`, `src/game/beats/synthesis/grade.ts:299` |
| 9 | Trace beat, guided rungs L0 to L2 (ADJACENT SURFACE, UNREACHABLE) | `src/game/beats/trace/GuidedCanvas.tsx` | None | Nothing, zero importers outside `src/beats/trace/` | Trace a structure onto guides | No | No | No | No | `gradeDrawing`, `src/game/beats/trace/recognise.ts:275` |
| 10 | Trace beat, freehand rung L3 (ADJACENT SURFACE, UNREACHABLE) | `src/game/beats/trace/FreehandCanvas.tsx` | None | Same as row 9 | Draw a structure from nothing | Yes, Undo and Clear | No | Yes, Clear | No | `gradeDrawing`, `src/game/beats/trace/recognise.ts:275` |
| 11 | Courses lesson player (ADJACENT SURFACE) | `src/game/lesson/LessonPlayer.tsx` | `#/courses/<courseId>/<topicId>` | Courses tab topic list | Seven curriculum kinds, three stubbed | No | No | No, locks on grade | No | `gradeAttempt`, `packages/curriculum/src/grading.ts:134` |
| 12 | Onboarding placement quiz (ADJACENT SURFACE) | `src/game/onboarding/PlacementStep.tsx` | `#/start/placement` | Onboarding flow, legacy `#/start/quiz` | Real chemistry, reuses `ProblemView` | No | No | No, one shot per question | No | `reduceQuiz`, `packages/curriculum/src/quiz/machine.ts:287` |
| 13 | Cards review session (ADJACENT SURFACE) | `src/game/cards/ui/ReviewSession.tsx` | `#/cards`, legacy `#/review` | Cards tab | Recall, then self-rate | No | No | No | No | Nothing, self-graded |
| 15 | Flashcard composer (ADJACENT SURFACE, ungraded) | `src/game/cards/ui/CardComposer.tsx` | `#/cards` | The Cards landing's compose control | Write a three-sided reaction card | No | No | Yes, per side, until Save | No | Nothing, the student authors it |
| 16 | pKa settings editor (ADJACENT SURFACE, UNREACHABLE) | `src/game/settings/PkaSettings.tsx` | None | Nothing, the component has zero importers | Edit the pKa ladder to match a professor | No, but a Clear my changes reset | No | Yes, per value | No | Nothing itself, but it FEEDS `judgeSort` |
| 17 | Pilot trainer screen (TRUE ARROW-PUSHING, DEV ROUTE) | `src/game/tabs/trainer/engine/TrainerScreen.tsx` | `#/gallery/pilot-trainer`, dev only | Typed by hand, nothing links to it, per the gallery-route rule | Resonance with tapered arrows, reaction with the arrowless electron gesture | Yes, an UNDO chip wired to the machine | Yes, a scrubber over the student's own recorded steps | Yes, a REDRAW chip, absent on the won screen | Yes on resonance only, tapered filled outline | `gradeDrawing`, `src/game/tabs/trainer/grade.ts:72`, unchanged |
...
- `src/game/tabs/trainer/TrainerTab.tsx` (930 lines, component at `:220`)

Supporting files:

- `blueberry_game/apps/web/src/tabs/trainer/DrawCanvas.tsx` (DELETED here by 291cd7c, the engine consolidation) (1225 lines, component at `:486`)
- `src/game/tabs/trainer/hitLayout.ts` (563 lines, targets and the hit tester)
- `src/game/tabs/trainer/grade.ts` (108 lines, the verdict)
- `src/game/tabs/trainer/equivalence.ts` (144 lines, canonical arrow keys)
- `src/game/tabs/trainer/distractors.ts` (154 lines, Tier 2 authored copy)
- `src/game/tabs/trainer/mistakes.ts` (the mistake journal, a localStorage rendering cache that
  seeds the Cards review queue, never an entitlement)
- `src/game/tabs/trainer/feedbackSound.ts` (the synthesised wrong-answer blip, paired with the
  failure animation)
- `src/game/tabs/trainer/CanvasBackdrop.tsx` (the water behind the molecules; it is what makes
...
- `src/game/render/layout/layout.ts` (where each atom of a state sits, the coordinates chem-core
  deliberately does not carry)
- `src/game/render/svg/MoleculeSvg.tsx` (the PLAYBACK renderer, not the draw canvas)
- `src/game/demo/useStepProgress.ts` (the animation clock, supplying `play` and `scrub`)
- `src/game/app/ui/Press.tsx` (the shared press button, imported at `TrainerTab.tsx:50`; the
  press contract is Part 6 item 5)
- `src/game/app/useHashRoute.ts` (`navigate`, imported at `TrainerTab.tsx:72`; it assigns only to
...
- `src/game/mascot/Berry.tsx` (the component, `TrainerTab.tsx:51`)
- `src/game/mascot/berryBehaviour.ts` (the behaviour type, `TrainerTab.tsx:52`)
- `src/game/mascot/berryMood.ts` (the mood type, `TrainerTab.tsx:53`)
- `src/game/mascot/berryCostume.ts` (`costumeForSurface`, `TrainerTab.tsx:54`)
- `src/game/mascot/berryReaction.ts` (`reactionFor` and `SETTLED_AFTER_MISS`, `TrainerTab.tsx:55`)

Route: `#/trainer`. The tab id is declared at `src/game/app/routes.ts:86`, parsed at
`routes.ts:158-160`, and rendered at `src/game/app/Shell.tsx:110-111`. This is the ONLY tab
...
- `src/game/tabs/pathway/PathwayTab.tsx:933-939` (`hrefForPlayable`, which builds
  `?reaction=<id>#/trainer` for a `reaction` link)
- `src/game/tabs/trainer/TrainerTab.tsx:567-570` (the `ProblemBrowser` mount,
  `onPick={openPlayable}`)
- `src/game/tabs/trainer/ProblemBrowser.tsx:22` (the picker component itself, which calls
  `onPick(node.playable)` at `:29`; the opener chip reads `<current title> · change` at `:43`)
- `src/game/tabs/trainer/TrainerTab.tsx:516-535` (`openPlayable`, the exhaustive switch)
...
- CURVED ARROWS: **yes.** Three draw sites in this instance, all in
  `blueberry_game/apps/web/src/tabs/trainer/DrawCanvas.tsx` (DELETED here by 291cd7c, the engine consolidation): the committed arrow at `:1071-1072` (a `--card` halo at
  ...
  `curveAway` at `:364-367`, over `bowAwayFrom` at `src/game/tabs/trainer/hitLayout.ts:413-436`.
...
- CURVED-ARROW COLOUR, named separately because "change how a curved arrow looks" most often means
  ...
  the arrows use are declared in the light block of `src/game/theme.css`: `--primary` at `:112`,
  ...
  at `src/game/tokens.css:170` and `:184`; `--electron-glow` has no primitive and is authored
  directly in `theme.css`. Which token goes where: the committed arrow stroke is `--primary`
  (``blueberry_game/apps/web/src/tabs/trainer/DrawCanvas.tsx` (DELETED here by 291cd7c, the engine consolidation):1072`) over a `--card` halo (`:1071`), the rejected ghost
  ...
  (`src/game/render/svg/MoleculeSvg.tsx:233` and `:138`).
- GRADING: `gradeDrawing(step, mechanism.arrows)` at `src/game/tabs/trainer/grade.ts:72`, called
  from `TrainerTab.tsx:417`. Returns `DrawVerdict` (`src/game/tabs/trainer/grade.ts:41-45`). Tier 1 legality comes from
  chem-core's `arrowLegalityFindings` (`src/game/tabs/trainer/grade.ts:73`), Tier 1 copy from `@blueberry/feedback`, and
  Tier 2 from `matchDistractor` (`src/game/tabs/trainer/distractors.ts:152-153`), called at
  `TrainerTab.tsx:453`.

#### Every arrow-drawing site in `src/game/`, as a CLOSED list
...
|---|---|---|---|
| `blueberry_game/apps/web/src/tabs/trainer/DrawCanvas.tsx` (DELETED here by 291cd7c, the engine consolidation) | `:880` (`draw-arrowhead`), `:883` (`draw-arrowhead-warn`) | `:1072`, `:1134`, `:1208` | The student's electron-flow arrows: committed, rejected ghost, in flight |
| `src/game/render/svg/MoleculeSvg.tsx` | `:137` (`arrowhead`, its OWN marker, a different id and different geometry from DrawCanvas's) | `:236` | The playback electron-flow arrows, on the same steps |
| `src/game/beats/trace/GuidedCanvas.tsx` | `:263-267` (a template-literal id, `` `${markerId}-head` ``) | `:403` | **NOT an electron-flow arrow.** See below |
...
|---|---|---|
| Committed arrow, ``blueberry_game/apps/web/src/tabs/trainer/DrawCanvas.tsx` (DELETED here by 291cd7c, the engine consolidation):1071-1072` | 2, a `--card` halo then a `--primary` stroke | `{7}` then `{3}` |
| Rejected ghost, ``blueberry_game/apps/web/src/tabs/trainer/DrawCanvas.tsx` (DELETED here by 291cd7c, the engine consolidation):1133-1134` | 2, halo then a dashed `--warn` stroke | `{7}` then `{3}` |
| In flight, ``blueberry_game/apps/web/src/tabs/trainer/DrawCanvas.tsx` (DELETED here by 291cd7c, the engine consolidation):1182` and `:1183-1209` | 2, halo then stroke | `{8}` then `{3.5}` (at `:1192`) |
| Playback, `src/game/render/svg/MoleculeSvg.tsx:229-241` | 1, no halo | `{3.5}` (at `:234`) |
| Tracing guide, `src/game/beats/trace/GuidedCanvas.tsx:396-405`, NOT electron flow | 1 | `{5}` (at `:400`) |
...
|---|---|
| Its COLOUR, live | `src/game/theme.css:112` (`--primary`), `:281` (`--electron-glow`), `:305` (`--warn`), dark overrides at `:514`, `:558`, `:567` |
| Its colour one layer down, the primitive ramp | `src/game/tokens.css:170` and `:184`. `--electron-glow` has no primitive and is authored directly in `theme.css` |
| Which token the drawn arrow spends | ``blueberry_game/apps/web/src/tabs/trainer/DrawCanvas.tsx` (DELETED here by 291cd7c, the engine consolidation):1072` (stroke), `:1071` (halo), `:1134` (rejected ghost), `:1066-1067` (arrowless electron dots) |
| Which token the PLAYBACK arrow spends | `src/game/render/svg/MoleculeSvg.tsx:233` and the marker fill at `:138` |
| Its thickness | The `strokeWidth` props listed in the paragraph above. Both copies, or draw and playback disagree |
| Its arrowhead shape | ``blueberry_game/apps/web/src/tabs/trainer/DrawCanvas.tsx` (DELETED here by 291cd7c, the engine consolidation):880` and `:883`, and separately `src/game/render/svg/MoleculeSvg.tsx:137`. These are three different marker definitions and none of them is shared |
| How far it bows, on the draw canvas | `src/game/tabs/trainer/hitLayout.ts:413-436` (`bowAwayFrom`), with `BOW_PX = 34` at ``blueberry_game/apps/web/src/tabs/trainer/DrawCanvas.tsx` (DELETED here by 291cd7c, the engine consolidation):213` |
| How far it bows, in playback | `src/game/render/layout/stepScene.ts:245-255` and `src/game/render/svg/MoleculeSvg.tsx:220-224`. A DIFFERENT rule, see Part 6 item 1 and D11 |
| Where it lands on the atom | `landingOnRim` at `src/game/tabs/trainer/hitLayout.ts:439`, with `LAND_GAP = 16` at `DrawCanvas.tsx:223` |
...
- `src/game/tabs/trainer/TrainerTab.tsx`

Supporting files:

- `src/game/demo/sequences.ts` (34 entries, `TRAINER_SEQUENCES` declared at `:5006`)
- everything listed under instance 1

Route: `?sequence=<id>#/trainer`. Read at `TrainerTab.tsx:169`, consumed at `TrainerTab.tsx:226`.
No hash of its own.

Wired at `src/game/tabs/pathway/PathwayTab.tsx:933-939` (a `sequence` link becomes
`?sequence=<id>#/trainer`) and at `src/game/tabs/trainer/TrainerTab.tsx:521-523`
...
- GRADING: `gradeDrawing`, `src/game/tabs/trainer/grade.ts:72`. Same path as instance 1.
...
- `src/game/demo/resonance.ts` (3 entries, `RESONANCE_HUNT` declared at `:138`)
- everything listed under instance 1
...
(`src/game/demo/pathwayMap.ts:23`); the translation happens at `PathwayTab.tsx:937`.

Wired at `src/game/tabs/pathway/PathwayTab.tsx:933-939` and at
`src/game/tabs/trainer/TrainerTab.tsx:524-526`.
...
- GRADING: `gradeDrawing`, `src/game/tabs/trainer/grade.ts:72`. Success copy comes from the
...
place, `src/game/app/Shell.tsx:111`, as `<TrainerTab reducedMotion={reducedMotion} />`. A grep
...
- `src/game/beats/mcq/McqRunner.tsx` (runner, component at `:99`)

Supporting files:

- `src/game/beats/mcq/McqBeatView.tsx` (the question view, component at `:107`)
- `src/game/beats/mcq/grade.ts` (`gradeMcq` at `:117`, `revealHeading` at `:169`)
- `src/game/beats/mcq/session.ts` (playing a RUN of easy beats, pure, no React and no clock)
- `src/game/beats/mcq/content.ts` (the authored questions, and nothing but data)
- `src/game/beats/mcq/card.ts` (turns a missed MCQ into a Cards offer, builds and saves nothing)
- `src/game/beats/mcq/authoring.ts` (the authoring contract made executable, and the one importer
  of `voiceViolations`; see Part 6 item 7)
- `src/game/beats/mcq/index.ts` (the barrel, so an integrating surface writes one path)
- `src/game/beats/BeatRunner.tsx` (the host, component at `:105`)
- `src/game/beats/template.ts` (the plan, `planLesson` at `:123`)
- `src/game/beats/RecipeStrip.tsx`, `src/game/beats/ChipPress.tsx`,
  `src/game/beats/beat-chrome.css`

Route: `#/lesson/<node>`. Parsed at `src/game/app/routes.ts:152`, built by `hrefForLesson`
(`routes.ts:168-170`), rendered by `src/game/App.tsx:99-109`, which lazily mounts `BeatRunner`.

Wired to a lesson node at `src/game/beats/template.ts:130-132` (`planLesson` pushes a
`recognise` step when `mcqBeatsForNode(node)` is non-empty) and mounted at
`src/game/beats/BeatRunner.tsx:202-214`, lazily imported at `BeatRunner.tsx:82`. Reached from the
...
- GRADING: `gradeMcq(attempt)` at `src/game/beats/mcq/grade.ts:117`, returning `McqReveal`
  (`src/game/beats/mcq/grade.ts:67-107`), not a `BeatResult`. See Part 4.
...
- `src/game/beats/match/MatchBoard.tsx` (587 lines, component at `:143`)

Supporting files:

- `src/game/beats/match/board.ts` (443 lines, the reducer, `beatResultFor` at `:402`,
  `boardVerdict` at `:369`, `causeForBoard` at `:384`, `cardForMiss` at `:427`)
- `src/game/beats/match/boards.ts` (the authored boards)
- `src/game/beats/match/spec.ts`
- `src/game/beats/match/reasons.ts` (`messageForLanding` at `:71`, `messageForMiss` at `:92`)
- `src/game/beats/match/match.css`
- `src/game/beats/match/index.ts`
- `src/game/beats/BeatRunner.tsx`

Route: `#/lesson/<node>`, same handling as instance 5.

Wired at `src/game/beats/template.ts:133-135` and mounted at
`src/game/beats/BeatRunner.tsx:215-224`, lazily imported at `BeatRunner.tsx:83`. The board for a
node is selected by `matchBoardsForNode` at `BeatRunner.tsx:70-72`.

Nodes served: `u5-syn-diol` (`src/game/beats/match/boards.ts:65`), `u9-pka` (`boards.ts:125`),
...
- UNDO: **no.** There is no undo and no reset. A wrong pair is refused and journalled as a miss, and
  a landed pair stays landed. The rationale is written at `src/game/beats/match/board.ts:396-401`.
- STEP REPLAY: **no.**
- REDRAW: **no**, for the reason above.
- CURVED ARROWS: **no.**
- GRADING: `beatResultFor(spec, state, input)` at `src/game/beats/match/board.ts:402`, called
...
- `src/game/beats/sort/SortBeatView.tsx` (556 lines, component at `:141`)

Supporting files:

- `src/game/beats/sort/judge.ts` (277 lines, `judgeSort` at `:127`, `SortJudgement` at `:61-73`)
- `src/game/beats/sort/board.ts`
- `src/game/beats/sort/ladders.ts`
- `src/game/beats/sort/sort.css`
- `src/game/beats/sort/index.ts`
- `src/game/beats/BeatRunner.tsx`

Route: `#/lesson/<node>`, same handling as instance 5. Concretely `#/lesson/u5-oxidation`,
`#/lesson/u8-ladder`, `#/lesson/u10-basicity`.

Wired at `src/game/beats/template.ts:136-139`, which reads `LADDER_FOR_NODE`
(`src/game/beats/template.ts:76-80`), and mounted at `src/game/beats/BeatRunner.tsx:225-234`,
...
- CURVED ARROWS: **no.**
- GRADING: `judgeSort(content, board, context)` at `src/game/beats/sort/judge.ts:127`, called at
  `SortBeatView.tsx:304-317`. Returns `SortJudgement` (`src/game/beats/sort/judge.ts:61-73`), which WRAPS a `BeatResult`
...
- **THE STUDENT-CONFIGURABLE INPUT, and it is the one thing about this beat a reader will not guess.**
  The pKa table this ladder is judged against is not fixed. `src/game/beats/sort/SortBeatView.tsx:158`
  ...
  from `src/game/settings/pka.ts`. So a student's own pKa numbers reach a grader on a routable
  surface. The editor for those numbers is `src/game/settings/PkaSettings.tsx`, which is instance
...
- `src/game/beats/synthesis/SynthesisGapBeat.tsx` (389 lines, component at `:161`)

Supporting files:

- `src/game/beats/synthesis/grade.ts` (434 lines, `gradeSynthesisGap` at `:299`,
  `beatCauseForCurriculumCause` at `:86`, `explainSynthesisResult` at `:381`)
- `src/game/beats/synthesis/corpus.ts` (`synthesisGapsForNode` at `:568`)
- `src/game/beats/synthesis/problem.ts` (the gap itself: a real multistep route with exactly one
  blank in it)
- `src/game/beats/synthesis/parse.ts` (turns what a student TYPED into reagent tokens the
  curriculum checker can compare; this is what makes the top rung fair)
- `src/game/beats/synthesis/cards.ts` (gap to Cards offer, and the bare default in D6 is here)
- `src/game/beats/synthesis/speech.ts` (the spoken-answer SEAM; nothing in it recognises speech)
- `src/game/beats/synthesis/structures.ts` (what a reactant gap's chips stand for, built through
  chem-core's real constructors)
- `src/game/beats/synthesis/beats.ts` (the adapter that makes a gap satisfy `beats/types.ts`)
- `src/game/beats/synthesis/index.ts` (the barrel: component, corpus, grader)
- `src/game/beats/BeatRunner.tsx`

Route: `#/lesson/<node>`, same handling as instance 5. Concretely `#/lesson/u3-sequencing`,
`#/lesson/u9-retro`, `#/lesson/u14-orthogonal`.

Wired at `src/game/beats/template.ts:140-142` and mounted at
`src/game/beats/BeatRunner.tsx:235-244`, lazily imported at `BeatRunner.tsx:85-87`.

Node coverage in the corpus: `u3-sequencing` at `src/game/beats/synthesis/corpus.ts:48`, `:115`
...
- GRADING: `gradeSynthesisGap(input)` at `src/game/beats/synthesis/grade.ts:299`. Returns a real
...
- `src/game/beats/trace/GuidedCanvas.tsx` (480 lines, component at `:87`)

Supporting files:

- `src/game/beats/trace/TraceBeatView.tsx` (278 lines, host component at `:94`, guided branch at
  `:213`, Check at `:262`, `outcomeLine` at `:74-92`)
- `src/game/beats/trace/recognise.ts` (346 lines, `recognise` at `:182`, `gradeDrawing` at `:275`,
  `toBeatResult` at `:314`, `guidedCause` at `:344`, `TraceOutcome` at `:261-264`)
- `src/game/beats/trace/geometry.ts`
- `src/game/beats/trace/target.ts`
- `src/game/beats/trace/render.ts`
- `src/game/beats/trace/content.ts`
- `src/game/beats/trace/index.ts`
- `src/game/beats/trace/PressButton.tsx` (this subtree's own press button, component at `:52`)
...
`src/game/beats/trace/` returns zero hits. The only importers anywhere are the three test files
above, and none of them imports a component. Separately, `ResolvedBeat`
(`src/game/beats/template.ts:86-90`) has four members, `mcq | match | sort | synthesis`, and the
string `trace` does not occur anywhere in `src/game/beats/template.ts`, so `planLesson` cannot
...
`src/game/lesson/ProblemView.tsx:90-101` currently stubs out with an apology. Related:
`src/game/beats/types.ts:121-128` declares SEVEN beat kinds and three of them (`mechanism`,
...
- GRADING: `gradeDrawing(target, drawn)` at `src/game/beats/trace/recognise.ts:275`, called from
  ...
  (`src/game/beats/trace/recognise.ts:261-264`) is a three-way union collapsed to a `BeatResult`
  by `toBeatResult` at `src/game/beats/trace/recognise.ts:314`.
...
- `src/game/beats/trace/FreehandCanvas.tsx` (315 lines, component at `:65`)

Supporting files: identical to instance 9. Mounted only by
`src/game/beats/trace/TraceBeatView.tsx:211`.
...
- UNDO: **yes**, and it is the most complete undo in the product: a real stack, on a real drawing.
  ...
  said it was. `src/game/beats/synthesis/SynthesisGapBeat.tsx:126-131` offers
...
- CURVED ARROWS: **no.**
- GRADING: `gradeDrawing`, `src/game/beats/trace/recognise.ts:275`. Same as instance 9.
...
- `src/game/lesson/LessonPlayer.tsx` (519 lines, component at `:138`)

Supporting files:

- `src/game/lesson/ProblemView.tsx` (445 lines, component at `:63`, the answer-kind switch at
  `:65-118`)
- `src/game/lesson/Feedback.tsx` (`Explain` at `:25`, the result mapping at `:71`)
- `src/game/lesson/RewardMoment.tsx`
- `src/game/lesson/LessonVideo.tsx`
...
- `src/game/tabs/courses/CoursesTab.tsx` (the host, mounts the player at `:350-357`)

Route: `#/courses/<courseId>/<topicId>`, for example `#/courses/orgo_2/aromaticity`. The tab is parsed
at `src/game/app/routes.ts:158-160` with the rest of the path handed through as `rest`
(`src/game/app/Shell.tsx:118-119`), and `src/game/tabs/courses/CoursesTab.tsx:340-357` turns
`rest` into a course and a topic. The player has no route of its own.

Wired to a lesson at `src/game/tabs/courses/CoursesTab.tsx:350-357`. It is NOT reachable from a
...
`src/game/lesson/ProblemView.tsx:65-118` switches on `problem.answer.kind`. `multiple_choice`
...
- `src/game/onboarding/PlacementStep.tsx` (495 lines, component at `:118`)

Supporting files:

- `src/game/lesson/ProblemView.tsx` (handed the problem at `PlacementStep.tsx:375`)
- `src/game/onboarding/StructureFigure.tsx` (the drawn answer tiles, used at
  `PlacementStep.tsx:485`)
- `src/game/onboarding/figures.ts` (the per-question figure data)
- `src/game/onboarding/Onboarding.tsx` (the host, the placement case at `:293-304`)
- `src/game/onboarding/flow.ts` (`placement` in the step list at `:48`, the legacy alias at
  `:63-64`, the progress band at `:110-123`)
- `src/game/onboarding/Frame.tsx`
- `src/game/onboarding/copy.ts`
- `src/game/onboarding/icons.tsx`
- `src/game/onboarding/onboarding.css`
- `packages/curriculum/src/quiz/machine.ts` (`createQuiz` at `:269`, `reduceQuiz` at `:287`; both
  imported at `src/game/onboarding/PlacementStep.tsx:48`)

Route: `#/start/placement`. Parsed at `src/game/app/routes.ts:149`, built by `hrefForOnboarding`
(`routes.ts:177-179`), rendered by `src/game/App.tsx:90-97`. The legacy hash `#/start/quiz`
normalises to it at `src/game/onboarding/flow.ts:63-64`.

Wired at `src/game/onboarding/Onboarding.tsx:293-304`. The quiz itself is created at
...
- GRADING: not graded in this file. `reduceQuiz` at `packages/curriculum/src/quiz/machine.ts:287`
  ...
  `src/game/onboarding/PlacementStep.tsx:131` (`advance`) with the two events at `:137`
...
- `src/game/cards/ui/ReviewSession.tsx` (259 lines, component at `:93`)

Supporting files:

- `src/game/cards/ui/CardsHome.tsx` (the host, component at `:75`. It is a FOUR-FACE switch on
  ...
  (`src/game/cards/ui/DeckTray.tsx`) at `:118-133`, the per-deck card list; `landing` at
  `:137-146` mounts `CardsLanding` (`src/game/cards/ui/CardsLanding.tsx`) at `:139-145`, the face
  a student actually arrives on, which reaches the composer through `onCompose` at `:144`)
- `src/game/cards/ui/CardFace.tsx` (the card, structures included, component at `:119`)
- `src/game/tabs/cards/CardsTab.tsx` (mounts `CardsHome` at `:43`)
- `src/game/cards/Recommendation.tsx` (the mistake-derived deck, `AuthoredExplanation` at `:166`,
  `ARROW_KEY_PATTERN` at `:200`, `arrowFromKey` at `:215`)

Route: `#/cards`, declared at `src/game/app/routes.ts:87` and rendered at
`src/game/app/Shell.tsx:114-115`. The legacy hash `#/review` redirects here at `routes.ts:156`.

Wired at `src/game/cards/ui/CardsHome.tsx:93-101`, the `review` face of the four-face switch
...
- GRADING: **none.** The student rates themselves. `rateCurrent(state, rating)` at
  ...
  `ReviewSession.tsx:164`. The scheduler is `src/game/cards/scheduler.ts` (238 lines), named here
...
- `blueberry_game/apps/web/src/tabs/trainer/TrainerTools.tsx` (DELETED here by 291cd7c, the engine consolidation) (325 lines, `TrainerTools` at `:31`, `Scratchpad` at
  `:216`, mounted at `:88`)
...
- `src/game/cards/ui/CardComposer.tsx` (586 lines, `export function Composer` at `:147`. The
...
- `src/game/cards/ui/composer.ts` (the logic beside the view: `EMPTY_SIDES`, `SIDE_ORDER`,
  `canSave`, `cardFromDraft`, `newDeckId`, `setSide`, all imported at `CardComposer.tsx:69-80`)
- `src/game/cards/ui/CardsHome.tsx` (the host, the `composer` face at `:103-104`)
- `src/game/cards/ui/CardsLanding.tsx` (where a student presses to get here, reaching the
  composer through `onCompose` at `CardsHome.tsx:144`)
- `src/game/cards/store.ts` (`decks`, the default `DeckSource`, imported at `CardComposer.tsx:68`)
- `src/game/cards/types.ts` (`DeckId`, `DeckSource`, `ReactionSide`, `ReactionSides`)

Route: `#/cards`, the same hash as instance 13, because this is a FACE of `CardsHome` rather than a
route of its own. Declared at `src/game/app/routes.ts:87`, rendered at
`src/game/app/Shell.tsx:114-115`.

Mount chain, all three links stated because the composer is otherwise unfindable from the route:
`src/game/cards/ui/CardsHome.tsx:39` imports it, `:103-104` mounts it in the `composer` face, and
...
structure-drawing canvas, and beside `src/game/lesson/ProblemView.tsx:90-101`, which stubs the
...
- `src/game/settings/PkaSettings.tsx` (507 lines, `export default function PkaSettings` at `:105`)

Supporting files:

- `src/game/settings/pka.ts` (the store and the rules: the `pkaSettings` store, `pkaValueFor`,
...
- `src/game/beats/sort/SortBeatView.tsx` (the CONSUMER, see below)
- Tests: `apps/web/test/pka-settings.test.ts`

Route: **NONE.** The COMPONENT has zero importers anywhere in `apps/web/src`. A grep for
`PkaSettings` outside `src/game/settings/` returns exactly two hits, both in
`src/game/beats/sort/SortBeatView.tsx` (`:91` and `:126`), and both are the TYPE
`PkaSettingsSnapshot`, imported from the sibling store `src/game/settings/pka.ts` at `:92`, never
the component. Widening that grep to `settings/pka` adds only that same import line and a comment at
`src/game/beats/sort/index.ts:10`, which exists to say the pKa layer is deliberately NOT
...
3. **Every call site of all five, in the whole of `apps/web/src`, is inside
   `src/game/settings/PkaSettings.tsx`**: `clearAllOverrides` at `:230-231`, `setPreset` at
   `:269` and `:316-317`, `setOverride` at `:392`, `clearOverride` at `:477` and `:481`. `reset` has
   zero callers anywhere.
4. The one other importer of the module reads and never writes. `src/game/beats/sort/SortBeatView.tsx:154-156`
   passes `pkaSettingsStore.subscribe` and `getSnapshot` to `useSyncExternalStore`, and nothing else.
5. That read reaches the grader: `src/game/beats/sort/SortBeatView.tsx:158` is
   `const table = settings ?? liveSettings`, feeding `judgeSort`
   (`src/game/beats/sort/judge.ts:127`) on the routable sort ladder of instance 7.
...
`src/game/beats/sort/index.ts:10` shows the boundary was drawn deliberately, saying the pKa layer
is NOT re-exported through the sort barrel because `src/game/settings/pka.ts` is meant to be the
...
- `src/game/tabs/trainer/engine/TrainerScreen.tsx`, a self-contained full-viewport component
...
Supporting files, all under `src/game/tabs/trainer/engine/`:
...
Route: `#/gallery/pilot-trainer`, dev only, handled by the gallery branch in `src/game/App.tsx`.
...
- GRADING: `gradeDrawing`, `src/game/tabs/trainer/grade.ts:72`, deliberately unchanged. An
...
The parser is `parseHash` at `src/game/app/routes.ts:140-162`. The dispatchers are
`src/game/App.tsx:72-115` and `src/game/app/Shell.tsx:107-146`.
...
| `#/start/quiz` | Normalises to `placement` | `src/game/onboarding/flow.ts:63-64` |
...
`src/game/app/routes.ts:130-135`:

- `#/gallery/berry`, the mascot workbench. Component at `src/game/mascot/BerryGallery.tsx:70`
  (247 lines). The href builder is `hrefForGallery` at `src/game/app/routes.ts:182-184`, and the
  comment there says it is typed by hand and nothing in the shell renders it. It is exempt from the
  onboarding redirect at `src/game/App.tsx:47-50`, so a critic with a fresh profile can reach it.
...
- `blueberry_game/apps/web/src/critic-main.tsx` (a harness entry point that never moved), 40 lines. It reads `?s=` from the query and renders
  `src/game/lesson/RewardMoment.tsx` with a hardcoded receipt by default, or
  `src/game/tabs/feed/FeedTab.tsx` when `?s=feed`.
...
- **This is the only way to render `FeedTab` at all.** A grep for `FeedTab` and `tabs/feed` across
  ...
  the number: two are the harness (``blueberry_game/apps/web/src/critic-main.tsx` (a harness entry point that never moved):6` and `:26`) and the other six are
  the feed folder naming itself (`src/game/tabs/feed/FeedIcon.tsx:4`, `:6`, `:9`, and
  `src/game/tabs/feed/FeedTab.tsx:101`, `:102`, `:462`). So there is **no importer outside the
  feed folder except the critic harness**. `src/game/tabs/feed/FeedIcon.tsx:9` is worth reading
  ...
  `feed` member in `TabId` (`src/game/app/routes.ts:38-52`), so `#/feed` falls through
...
`src/game/tabs/trainer/TrainerTab.tsx:151` (the `URLSearchParams` construction) and at
...
document load to take effect, which is why `src/game/charge/ChargeGate.tsx:96-102` resolves
...
- `?auto=1`, `TrainerTab.tsx:152`, and also `src/game/App.tsx:38-40` where it suppresses the
  onboarding redirect
...
- `?primitive=arrow`, ``blueberry_game/apps/web/src/tabs/trainer/DrawCanvas.tsx` (DELETED here by 291cd7c, the engine consolidation):276`, which restores the
...
|---|---|---|---|---|
| `gradeDrawing` | `src/game/tabs/trainer/grade.ts:72` | `(step: MechanismStep, drawn: readonly ElectronFlowArrow[])` | `DrawVerdict` | **No, weaker.** No `correct_alternative_route`, and `not_requested` deliberately is NOT the registry's `valid_transformation_not_requested`. Adds a fifth non-result, `incomplete` |
| `missingArrows` | `src/game/tabs/trainer/grade.ts:105` | `(step, drawn)` | `readonly ElectronFlowArrow[]` | n/a. **DEAD.** A grep across `apps/` and `packages/` including tests returns its own definition and a stale `apps/web/tsbuild/src/tabs/trainer/grade.d.ts:49`, nothing else |
| `matchDistractor` | `src/game/tabs/trainer/distractors.ts:152` | `(step: MechanismStep, arrow: ElectronFlowArrow)` | `TrainerDistractor \| null` | n/a. A Tier 2 lookup, not a verdict |
| `gradeMcq` | `src/game/beats/mcq/grade.ts:117` | `(attempt: McqAttempt)` | `McqReveal` | **No.** `McqReveal` (`src/game/beats/mcq/grade.ts:67-107`) is a reveal payload carrying `matchedAnswer: boolean` and `firstMeeting: boolean`, not an outcome union |
| `beatResultFor` | `src/game/beats/match/board.ts:402` | `(spec: MatchBoardSpec, state: BoardState, input: BeatResultInput)` | `BeatResult` | **Type yes, behaviour no.** It always returns `kind: "correct"`, by the design note at `src/game/beats/match/board.ts:396-401`, so three of the four are unreachable here |
| `judgeSort` | `src/game/beats/sort/judge.ts:127` | `(content: SortContent, board: SortBoard, context: JudgeContext)` | `SortJudgement` | **Wraps yes.** `SortJudgement` (`src/game/beats/sort/judge.ts:61-73`) is `unfinished \| judged`, and the `judged` arm carries a full `BeatResult` plus a `tier` and an `Explanation` |
| `gradeSynthesisGap` | `src/game/beats/synthesis/grade.ts:299` | `(input: GradeGapInput)` | `BeatResult` | **Yes, all four** |
| `gradeDrawing` (the other one) | `src/game/beats/trace/recognise.ts:275` | `(target: TraceTarget, drawn: Graph)` | `TraceOutcome` | **No.** Three-way, `correct \| invalid \| undecided`, collapsed to two by `toBeatResult` at `src/game/beats/trace/recognise.ts:314` |
...
table that is not a constant: `src/game/beats/sort/SortBeatView.tsx:158` resolves it as
`const table = settings ?? liveSettings`, reading the live `pkaSettings` store from
`src/game/settings/pka.ts` (imported at `SortBeatView.tsx:88-92`). No other grader in the table
...
`src/game/settings/PkaSettings.tsx` (instance 16), has zero importers: the store's five mutators
(`src/game/settings/pka.ts:867`, `:875`, `:884`, `:890`, `:894`) are called from that file and
...
`src/game/beats/match/reasons.ts:71` and `messageForMiss` at `reasons.ts:92` produce the match
board's per-pair copy, and `beatCauseForCurriculumCause` at
`src/game/beats/synthesis/grade.ts:86` translates a curriculum cause into a beat shape cause.

**Two different exported functions are both called `gradeDrawing`**, with different signatures and
different return types: `src/game/tabs/trainer/grade.ts:72` and
`src/game/beats/trace/recognise.ts:275`. Nothing imports both, so this compiles, but a reader who
...
|---|---|---|---|
| Declared at | `src/game/beats/types.ts:603-615` | `src/game/tabs/trainer/grade.ts:41-45` | `packages/curriculum/src/grading.ts:81-119` |
...
| Compile-time guard against chem-core's `ResolutionKind` | **Yes, bidirectional**, at `src/game/beats/types.ts:632-633`, over the `Assert` and `NoExtras` pair at `:629-630` | **None** | **None** |
...
`ResolutionKindCoversBeatOutcome` at `src/game/beats/types.ts:632-633` check both directions, so
...
`src/game/tabs/trainer/grade.ts:1-28` is honest about why: chem-core has no function from a drawn
...
is why `src/game/lesson/Feedback.tsx:71` can render an authored explanation directly while the
trainer has to assemble one.

A fourth, smaller union sits under the trace beat: `TraceOutcome` at
`src/game/beats/trace/recognise.ts:261-264`, three-way, collapsed to `BeatResult` by
`toBeatResult` at `src/game/beats/trace/recognise.ts:314`. It is a converter rather than a fourth contract, listed here so
...
- `src/game/beats/sort/judge.ts:127`, inside `judgeSort`
- `src/game/beats/synthesis/grade.ts:299`, plus `beatCauseForCurriculumCause` at `:86`
- `src/game/beats/match/board.ts:402`, `beatResultFor`
- `src/game/beats/trace/recognise.ts:314`, `toBeatResult`
...
- `docs/reference/design-goals/blueberry_r9-lesson-mechanism_1788289491.png`
  ...
  **THE TAPERED-ARROW REFERENCE.** Opened and looked at during this audit. A cream page. The header
  ...
  (`src/game/tabs/trainer/TrainerTab.tsx:410-417`) and has no student-facing undo.
...
- `src/game/render/layout/stepScene.ts:245-255` sets the bow SIDE as
  `bow: index % 2 === 0 ? 1 : -1` at `:253`. That is arrow LIST INDEX PARITY, with no geometry input
  at all.
- `src/game/render/svg/MoleculeSvg.tsx:220-224` consumes that sign with a fixed magnitude of
  `0.55 * PX`, which is 39.6 px, uncapped and unfloored.
- `src/game/tabs/trainer/hitLayout.ts:413-436` (`bowAwayFrom`) caps and floors the magnitude at
...
- ``blueberry_game/apps/web/src/tabs/trainer/DrawCanvas.tsx` (DELETED here by 291cd7c, the engine consolidation):364-367` (`curveAway`) is the only caller of
...
**Magnitude**: playback is a flat 39.6 px with no cap, and `src/game/tabs/trainer/hitLayout.ts:419-429`
...
(`src/game/render/svg/MoleculeSvg.tsx:226-227`), while the draw canvas swings the landing onto the
atom rim with `landingOnRim` and `MIN_CHORD = 34` (`hitLayout.ts:439`, called from
``blueberry_game/apps/web/src/tabs/trainer/DrawCanvas.tsx` (DELETED here by 291cd7c, the engine consolidation):287`, `:325` and `:1130`).

What a student sees: `src/game/tabs/trainer/TrainerTab.tsx:428` flips to play mode the instant an
...
- `src/game/render/svg/MoleculeSvg.tsx:34-48`: `PX = 72`, `ATOM_R = 21`, `ATOM_R_SMALL = 13`,
  `HIT_R = 22`, `px()` at `:45-48`, `radiusFor()` at `:41-43` with **two tiers**, H is 13 and
  everything else is 21.
- `src/game/tabs/trainer/hitLayout.ts:34-51`: the same three constants re-declared, plus
  `ATOM_R_LARGE = 26`, `toPx()` at `:39-41`, `atomRadius()` at `:43-51` with **three tiers**, H is 13,
  Br, I, Cl, S and P are 26, everything else is 21.
- ``blueberry_game/apps/web/src/tabs/trainer/DrawCanvas.tsx` (DELETED here by 291cd7c, the engine consolidation):280-282` (`elementRadius`) is a thin wrapper over
...
`src/game/tabs/trainer/hitLayout.ts:7-10` admits the copy in prose. The real drift is the radius:
...
``blueberry_game/apps/web/src/tabs/trainer/DrawCanvas.tsx` (DELETED here by 291cd7c, the engine consolidation):285-289` and `:1129-1130`) with `LAND_GAP = 16`
...
- `src/game/tabs/trainer/grade.ts:47-70` (`sourceKey`, `sinkKey`, `arrowKey`), raw ids, no
  canonicalisation.
- `src/game/tabs/trainer/equivalence.ts:104-125` (`canonicalArrowKey`), the same output grammar
...
- `src/game/cards/Recommendation.tsx:200` (`ARROW_KEY_PATTERN`) plus `arrowFromKey` at `:215`, the
  grammar written a third time, as a parser.
- `src/game/tabs/trainer/distractors.ts:35-149` (`DISTRACTORS_BY_STEP`), the grammar hand-written
...
(`src/game/tabs/trainer/grade.ts:89-90`) precisely so that a student who pushes the chemically
...
(`src/game/tabs/trainer/distractors.ts:152-153`). So a student who draws a wrong arrow from the
...
The fourth site, `missingArrows` at `src/game/tabs/trainer/grade.ts:105-107`, is also on the raw
key but is **DEAD**, as recorded in Part 4. It is a latent copy, not a second live bug. The journal
path is internally consistent: `src/game/tabs/trainer/TrainerTab.tsx:435` and `:455` store raw
keys and `src/game/cards/Recommendation.tsx:215` parses raw, so any fix must migrate `grade.ts`,
...
- `src/game/beats/trace/FreehandCanvas.tsx:92`: `commit` pushes `snapshotOf(next)`, the NEW state,
  with `[...previous.slice(-30), ...]`; `undo` at `:154-158` reads `past[past.length - 2] ?? EMPTY`.
- ``blueberry_game/apps/web/src/tabs/trainer/TrainerTools.tsx` (DELETED here by 291cd7c, the engine consolidation):299-306`: a mutable `strokesRef.current.pop()` on
...
`src/game/beats/trace/FreehandCanvas.tsx:160-163` sets `past` to `[]` without pushing, so a clear
...
- `src/game/theme.css:754-770`, `.press` and `.press:active`: `transform: scale(0.96)` plus
  `filter: brightness(0.92)`, transition 120 ms. Consumed by `src/game/app/ui/Press.tsx:66`.
- `src/game/beats/beat-chrome.css:114-116`, `.chip-press:active:not(:disabled)`:
  `translateY(var(--chip-press-travel))` with the box-shadow edge collapsing. Consumed by
  `src/game/beats/ChipPress.tsx:36`.
- `src/game/beats/trace/PressButton.tsx:61-91`: React `useState`, with
  `transform: pressed ? "translateY(2px) scale(0.98)" : "none"` at `:89` and a 90 ms transition at
  `:90`.
- ``blueberry_game/apps/web/src/tabs/trainer/TrainerTools.tsx` (DELETED here by 291cd7c, the engine consolidation):299-322`: `className="press"` plus raw `onPointerDown`
...
acknowledgement path, which is exactly what `src/game/theme.css:750-753` avoids by using
...
- `src/game/tabs/trainer/hitLayout.ts:60`:
...
- `packages/curriculum/src/explanation.ts:41`, `:49`, `:56`: `whatHappened` / `why` / `lookAt`. The
  ...
  not the student
- `src/game/tabs/trainer/distractors.ts:21-28`: `what` / `why` / `lookAt`
- `src/game/cards/Recommendation.tsx:166-179`: `what` / `why` / `lookAt` plus `tier`
...
`src/game/tabs/trainer/TrainerTab.tsx:841`, `:856`, `:879`, `:906` and `:920` render
`<Detail label="Look at" .../>`; `src/game/beats/sort/SortBeatView.tsx:531` renders
`<b>Look at</b>`; `src/game/lesson/Feedback.tsx:37` renders `<dt>Look at</dt>` inside `Explain`
(`:25`), fed at `:71`.
...
`src/game/beats/mcq/authoring.ts` imports it rather than reimplementing it, which is the right
pattern. But the trainer's own distractor copy (`src/game/tabs/trainer/distractors.ts`, 15
...
`src/game/beats/trace/render.ts:123-124` guards `length <= 1e-6`;
`src/game/render/svg/depth.tsx:161-162` guards with `|| 1`;
`src/game/onboarding/StructureFigure.tsx:69-70` guards with `|| 1`;
`src/game/tabs/trainer/hitLayout.ts:431-433` guards with an early return at `:418` when `len < 1`.

Planar distance `Math.hypot(a.x - b.x, a.y - b.y)`: `src/game/tabs/trainer/hitLayout.ts`
(private), `src/game/beats/trace/geometry.ts` (exported), and
`packages/interaction/src/geometry/units.ts` (unreachable, see item 6).

`lerp` versus `mix`: `src/game/render/layout/vec.ts` carries z and
`src/game/tabs/trainer/hitLayout.ts:30-32` does not, and the split is documented at
`hitLayout.ts:29`. That one is defensible and should stay.

Multiple-bond offsets are three unshared constant sets: `src/game/render/svg/depth.tsx:164` uses
`[-5.5, 5.5]` and `[-10, 0, 10]`; `src/game/onboarding/StructureFigure.tsx:26-27` uses
`DOUBLE_GAP = 3.1` and `TRIPLE_GAP = 4.4`, applied at `:87`; `src/game/beats/trace/render.ts:26`
...
- **`step: MechanismStep`**, at `src/game/demo/reactions.ts:41`,
  `src/game/demo/sequences.ts:36` and `src/game/demo/resonance.ts:39`. This IS the question.
...
- **`fromHints` and `toHints`**, at `src/game/demo/reactions.ts:42-43`,
  `src/game/demo/sequences.ts:38-39` and `src/game/demo/resonance.ts:40-41`. Where an author
...
- **`title`, `brief`, `successLine`, `foundLine`, `stepBrief`**, at
  `src/game/demo/reactions.ts:35-40`, `src/game/demo/sequences.ts:32-37` and
  `src/game/demo/resonance.ts:35-38`. Authored copy, one per problem, reviewed by a person, and a
  ...
  `src/game/tabs/trainer/TrainerTab.tsx:127` and `:148`.
- **Tier 2 distractor entries, keyed by the offending arrow**, at
  `src/game/tabs/trainer/distractors.ts:35-149`, matched at `:152-153`. The specific mistakes an
  ...
  covered above.
- **`step.identity.reactionCenters`**, read at ``blueberry_game/apps/web/src/tabs/trainer/DrawCanvas.tsx` (DELETED here by 291cd7c, the engine consolidation):1087-1095` to
...
- **Sequence step ordering and the per-step brief**, at `src/game/demo/sequences.ts:35-40`.
  Correctly nested: the ordering is the question. One wart:
  `src/game/tabs/trainer/TrainerTab.tsx:140` synthesises a non-final step's success line by regex,
  `item.stepBrief.replace(/^Step \d+ · /, "Done: ")`, which is authored copy transformed by code and
  should be an authored field.
- **The mastery rung and its guide style**, at `src/game/beats/types.ts:225`
  ...
  `level` prop at `src/game/App.tsx:103-107`, defaulting to 1 at
  `src/game/beats/BeatRunner.tsx:105`, so nothing above L1 is reachable anywhere today.

**Two things that are currently CODE and should be DATA.** `arrowless` is hardcoded `true` for every
sequence at `src/game/tabs/trainer/TrainerTab.tsx:141`, and `resonance` is hardcoded `true` for
every hunt at `:148`. Both are consumed as props at `:625` and resolved to a primitive at
``blueberry_game/apps/web/src/tabs/trainer/DrawCanvas.tsx` (DELETED here by 291cd7c, the engine consolidation):487`, which supports the two switches INDEPENDENTLY. So
...
**Element radius tier**, at `src/game/tabs/trainer/hitLayout.ts:43-51`, is chemistry data
...
`src/game/tabs/trainer/hitLayout.ts`, 563 lines of shell code reimplementing a ranking rule
...
**Why not `src/game/tabs/trainer/`.** Because that is where it lives now, and Part 6 is the
...
- **The projection.** `PX`, the atom radius tiers, scene to pixel, and element radius as ONE table.
  This kills the `src/game/render/svg/MoleculeSvg.tsx:34-48` versus
  `src/game/tabs/trainer/hitLayout.ts:34-51` split and the bromine r=26 versus r=21 replay drift.
- **Curve geometry as pure functions returning geometry, not path strings.** The bow (side rule AND
  ...
  `src/game/render/layout/stepScene.ts:253`'s index-parity bow, the audit's sharpest defect.
- **Target compilation.** Turning a `StepScene` plus a draft into the target list that
  ...
  `src/game/tabs/trainer/hitLayout.ts:60`'s uniform per-pointer slop.
- **The arrow-key grammar.** Exactly one encoder, one decoder and one canonicaliser, so
  `src/game/tabs/trainer/grade.ts`, the data keys in `src/game/tabs/trainer/distractors.ts`
  and the regex at `src/game/cards/Recommendation.tsx:200` cannot drift apart again.
- **Verdict shaping.** `DrawVerdict` construction with the same compile-time `Assert<NoExtras<...>>`
  proof against chem-core's `ResolutionKind` that `src/game/beats/types.ts:632-633` already
  demonstrates is workable.
- **The `Playable` normalisation** currently trapped inside
  `src/game/tabs/trainer/TrainerTab.tsx:110-149`, with `arrowless` and `resonance` read from the
...
- **SVG elements, markers, CSS, colour tokens, or the press language.**
  `blueberry_game/apps/web/src/tabs/trainer/DrawCanvas.tsx` (DELETED here by 291cd7c, the engine consolidation) and `src/game/render/svg/MoleculeSvg.tsx` stay in
...
- **The registries themselves.** `src/game/demo/reactions.ts`, `sequences.ts` and `resonance.ts`
...
`src/game/tabs/trainer/distractors.ts:153` through `canonicalArrowKey`. Those are the two
...
1. **`src/game/beats/types.ts:121`**, add the member to the `BeatKind` union. **And
   `src/game/beats/types.ts:130`, `BEAT_KINDS`**, the frozen runtime array beside it. This is the
...
2. **`src/game/beats/types.ts:141`**, `BUILT_BEAT_KINDS`. Add it only when the kind really is
   built, because this is what makes a coverage report tell the truth about itself.
3. **`src/game/beats/types.ts:160`**, `DEFAULT_LEVELS`, declaring which mastery rungs the kind can
   serve. Note D16 before spending long here: nothing above L1 is reachable today, because
   `src/game/App.tsx:103-107` mounts `BeatRunner` with no `level` prop.
4. **`src/game/beats/template.ts:86-90`**, add the arm to `ResolvedBeat`. **And
   `src/game/beats/template.ts:92`, `SLOT_FOR_KIND`**, which is a
   ...
   **`src/game/beats/template.ts:62`**: `"hook" | "recognise" | "connect" | "order" | "produce"`.
   Those five words are used as prose throughout Part 2; that line is where they are actually defined.
5. **`src/game/beats/template.ts:123-145`**, add a push block to `planLesson`, mirroring the four
...
6. **`src/game/beats/template.ts:160-162`**, `nodeHasBeat`. It is `planLesson(node) !== null`, so
...
7. **`src/game/beats/template.ts:282-298`**, `problemBadge`, add a case. Its bare `default` at
...
8. **`src/game/beats/BeatRunner.tsx`**, a `lazy()` import beside the four at `:82-87`, and a mount
...
9. **The corpus module**, one per kind, and the existing four are the pattern:
   `src/game/beats/mcq/content.ts`, `src/game/beats/match/boards.ts`,
   `src/game/beats/sort/ladders.ts`, `src/game/beats/synthesis/corpus.ts`. Authored data,
   keyed by node, and it is what step 5's condition asks about.
10. **The grader**, a new entry function returning a `BeatResult`
    (`src/game/beats/types.ts:603-615`). Part 4 lists the four that exist and what each one
...
11. **Only if the kind must also appear in the COURSES player**, which is a second and separate
    system: `src/game/lesson/ProblemView.tsx`'s answer-kind switch at `:65-118`, and
...
Cause. `PlayableLink` (`src/game/demo/pathwayMap.ts:20-30`) is a FOUR-kind union,
...
(`src/game/tabs/trainer/TrainerTab.tsx:146-148`) then does
...
`hrefForPlayable` at `src/game/tabs/pathway/PathwayTab.tsx:933-939` has always handled `beat`
...
`src/game/tabs/trainer/TrainerTab.tsx:516-535`, an exhaustive `switch` on `link.kind`, with
...
`src/game/tabs/trainer/TrainerTab.tsx:151` builds `URLSearchParams` at MODULE LOAD and
...
(`src/game/app/useHashRoute.ts:33-35`) assigns only to `window.location.hash`.
...
`src/game/charge/ChargeGate.tsx:81-101` documents the mirror image of this trap and is worth
reading beside it.
...
`src/game/App.tsx:99-108` mounts `BeatRunner` with
`onExit={() => navigate(hrefForTab("pathway"))}`. `BeatRunner` takes `onExit` as a prop
(`src/game/beats/BeatRunner.tsx:93`), so the route cannot know where the student came from. A
...
`src/game/charge/ChargeGate.tsx:281-295`, with `progress.startNode(...)` at `:284` and the two
`go(model.node.href)` calls at `:286` and `:292`. A beat is priced as `"concept"` by
`src/game/tabs/pathway/PathwayTab.tsx:926-930`.

`openPlayable` (`src/game/tabs/trainer/TrainerTab.tsx:527-529`) navigates straight to
`#/lesson/<id>` with no sheet and no spend. So the same lesson costs Charge from one entrance and is
free from the other.

Related and worth fixing in the same pass: `src/game/tabs/pathway/PathwayTab.tsx:936` hand-builds
the lesson href as a template literal rather than calling `hrefForLesson`
(`src/game/app/routes.ts:168-170`), so one route has two spellings in the codebase.
...
`src/game/beats/trace/TraceBeatView.tsx:74-92` (`outcomeLine`) switches over `TraceCauseId` with
...
surface is orphaned. A grep across `apps/web/src` excluding `src/game/beats/trace/` returns ZERO
importers of `TraceBeatView`, `GuidedCanvas`, `FreehandCanvas` or `beats/trace`, and the string
`trace` does not occur anywhere in `src/game/beats/template.ts`, so `ResolvedBeat`
(`src/game/beats/template.ts:86-90`) cannot carry a trace beat and `planLesson`
(`src/game/beats/template.ts:123-145`) can never
schedule one.
...
- `src/game/beats/synthesis/cards.ts:122-135`, a bare `default` at `:133-134` over the synthesis
...
- `src/game/tabs/pathway/pathwayState.ts:103-122` handles 4 of the 10 `EconomyEvent` kinds with
...
- `src/game/tabs/trainer/hitLayout.ts:379-396` (`targetAnchor`) switches over `HitTarget` with a
...
- `src/game/lesson/LessonPlayer.tsx:99-107` (`surfaceForProblem`) has a `default` at `:105-106`.
  It selects a mascot costume only, so the cost of a miss is the wrong outfit.

For contrast, `src/game/beats/template.ts:282-298` (`problemBadge`) also has a bare `default`, at
`:296-297`, and it is DELIBERATE and documented at `src/game/beats/template.ts:280`: an unknown answer kind gets the
...
the trainer's dispatch. **No test anywhere imports `src/game/tabs/trainer/TrainerTab.tsx`.**
...
`src/game/beats/match/boards.ts:180-184` authors a match board for node `u6-ir`, so
`nodeHasBeat("u6-ir")` (`src/game/beats/template.ts:160-162`) returns true. But `u6-ir` on the map
(`src/game/demo/pathwayMap.ts:147`) is a `gate` with NO `playable` field, so the problem browser
disables it and nothing reaches the board. `coverage()`
(`src/game/demo/pathwayMap.ts:348`) counts nodes that carry a `playable`, so it is wrong about
...
(`src/game/beats/types.ts:603-615`) is the specification's four exactly and is the only one with a
compile-time guard (`src/game/beats/types.ts:632-633`). `DrawVerdict`
(`src/game/tabs/trainer/grade.ts:41-45`) is what the arrow-pushing trainer uses, cannot express
...
shipped tester at `src/game/tabs/trainer/hitLayout.ts:538-563` uses a uniform per-pointer slop
...
Detailed in Part 6, item 1. `src/game/render/layout/stepScene.ts:253` picks the bow side by arrow
LIST INDEX PARITY, with no geometry input, while
`src/game/tabs/trainer/hitLayout.ts:435` picks it by distance from the scene centroid. For any
step whose arrows sit on the same side of the molecule, playback arcs the odd-indexed arrow through
it, and `src/game/tabs/trainer/TrainerTab.tsx:428` shows that playback to the student immediately
...
Detailed in Part 6, item 3. `src/game/tabs/trainer/grade.ts:89-90` tallies with
`canonicalArrowKey`, but `src/game/tabs/trainer/distractors.ts:152-153` looks up with the raw
...
Detailed in Part 6, item 4. `src/game/beats/trace/FreehandCanvas.tsx:92` caps the stack with
...
`src/game/tabs/trainer/TrainerTab.tsx:141`, and `resonance` is hardcoded `true` for every hunt at
`:148`, even though ``blueberry_game/apps/web/src/tabs/trainer/DrawCanvas.tsx` (DELETED here by 291cd7c, the engine consolidation):487` supports the two switches
...
`src/game/beats/types.ts:121-128` declares seven `BeatKind` members. `ResolvedBeat`
(`src/game/beats/template.ts:86-90`) has four. So `mechanism`, `resonance` and `trace` exist in
the type system and can never reach a lesson. `src/game/beats/types.ts:141` already names two of
...
Related and worth noting in the same breath: `src/game/lesson/ProblemView.tsx:90-101` stubs the
`structure` answer kind with a message saying the drawing canvas "arrives with the editor route",
while `src/game/beats/trace/` IS a working structure-drawing canvas with three test files. Two
...
`src/game/App.tsx:103-107` mounts `BeatRunner` with no `level` prop, and
`src/game/beats/BeatRunner.tsx:105` defaults it to 1. `src/game/beats/types.ts:160` declares
which rungs each beat kind can serve and `src/game/beats/types.ts:225` (`traceGuideStyle`) fades guides across L0 to
...
`src/game/app/routes.ts:150-151` carries a comment saying that `#/lesson` with no node "falls
through to the trainer". It does not. It falls through to the PATH tab, at
`src/game/app/routes.ts:161`, which is the same default every unrecognised hash takes, and which
...
address. The fix is one comment, in `src/game/app/routes.ts`. No behaviour changes.
...
|---|---|---|---|
| Instance 9, guided trace canvas | `src/game/beats/trace/GuidedCanvas.tsx` | Zero. Only `apps/web/test/traceContent.test.ts`, `traceGeometry.test.ts`, `traceRecognise.test.ts` | The structure-drawing surface `src/game/lesson/ProblemView.tsx:90-101` apologises for not having |
| Instance 10, freehand trace canvas | `src/game/beats/trace/FreehandCanvas.tsx` | Zero, same evidence | The same, at the top rung, plus the product's best undo |
| Instance 16, pKa settings editor | `src/game/settings/PkaSettings.tsx` | Zero for the COMPONENT. The only two hits for `PkaSettings` outside `src/game/settings/` are `src/game/beats/sort/SortBeatView.tsx:91` and `:126`, and both import the TYPE `PkaSettingsSnapshot` from `src/game/settings/pka.ts` | A grader input a student is supposed to control |

The two subsystems are `src/game/beats/trace/` and `src/game/settings/`, which import nothing
...
`src/game/beats/sort/SortBeatView.tsx:158` reads `const table = settings ?? liveSettings` from
the live `pkaSettings` store (`src/game/settings/pka.ts:901`), so the pKa table `judgeSort`
(`src/game/beats/sort/judge.ts:127`) marks a student against is meant to be configurable. **Today
...
their call sites inside `src/game/settings/PkaSettings.tsx`, and `reset` at `:894` is never
...
`src/game/settings/PkaSettings.tsx:1-11` states the reason the screen exists:
...
next: `src/game/beats/BeatRunner.tsx`, `src/game/beats/RecipeStrip.tsx`,
`src/game/beats/beat-chrome.css`, `src/game/beats/mcq/McqBeatView.tsx`,
`src/game/beats/template.ts`, `src/game/lesson/LessonPlayer.tsx`,
`src/game/lesson/LessonVideo.tsx`, `src/game/lesson/ProblemView.tsx`,
`src/game/onboarding/onboarding.css`, `src/game/tabs/feed/feed.css` and
`blueberry_game/apps/web/src/critic-main.tsx` (a harness entry point that never moved). By the time this file was finished the same session had also dirtied
`src/game/beats/match/MatchBoard.tsx`, `src/game/beats/sort/SortBeatView.tsx`,
`src/game/beats/synthesis/SynthesisGapBeat.tsx`, `src/game/beats/trace/TraceBeatView.tsx` and
`src/game/lesson/ReactionStrip.tsx`, so instances 6, 7, 8 and 9, and the ordered list in Part 7b,
...
`src/game/lesson/LessonPlayer.tsx` and `src/game/lesson/ProblemView.tsx` moved DURING the
...
performs pointer drags on `src/game/beats/match/MatchBoard.tsx` and types into
`src/game/beats/synthesis/SynthesisGapBeat.tsx:330-345`.
...
`src/game/render/svg/MoleculeSvg.tsx:41-43` and `src/game/tabs/trainer/hitLayout.ts:43-51`
...
`src/game/tabs/feed/FeedTab.tsx` is built, has no route, and is reachable only through
`apps/web/critic.html?s=feed` (it has no importer outside the feed folder except the harness at
``blueberry_game/apps/web/src/critic-main.tsx` (a harness entry point that never moved):6` and `:26`, and `feed` is not a member of `TabId` at
`src/game/app/routes.ts:38-52`; the full eight-hit grep is broken down in Part 3). Under `CLAUDE.md`'s
...
**Not read end to end.** `src/game/tabs/trainer/TrainerTab.tsx` (930 lines),
`blueberry_game/apps/web/src/tabs/trainer/DrawCanvas.tsx` (DELETED here by 291cd7c, the engine consolidation) (1225 lines) and
`src/game/beats/trace/GuidedCanvas.tsx` (480 lines) were read in the regions this inventory cites:
...
by counting entries after `src/game/demo/sequences.ts:5006`. One scout described `missingArrows`
(`src/game/tabs/trainer/grade.ts:105`) as live; it is dead. And an earlier pass flagged
`u3-sequencing`, `u9-retro` and `u14-orthogonal` as having no authored content; all three resolve, and
their gaps live in `src/game/beats/synthesis/corpus.ts` rather than in `beats.ts`.
