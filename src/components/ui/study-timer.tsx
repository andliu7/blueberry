...
export function StudyTimer() {
  ...
  const compactCard = (
    <section
      className="group flex h-full min-h-[190px] cursor-pointer flex-col rounded-2xl border border-slate-200 bg-white/80 p-5 shadow-sm transition hover:border-[#bbb1eb] hover:shadow-md focus:outline-none focus-visible:ring-2 focus-visible:ring-[#9989e2] dark:border-stone-700 dark:bg-stone-900/75"
      role="button"
      tabIndex={0}
  ...
  return <>
    ...
    {typeof document !== "undefined" && createPortal(<>
        ...
        {dialogOpen && <motion.div className="fixed inset-0 z-[120] grid place-items-center bg-slate-950/45 p-4 backdrop-blur-sm" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} onMouseDown={() => setDialogOpen(false)}>
          <motion.section role="dialog" aria-modal="true" aria-labelledby="study-timer-title" className="relative max-h-[calc(100dvh-1rem)] w-full max-w-3xl overflow-hidden rounded-[28px] border border-white/20 bg-[#171327]/95 p-5 text-white shadow-2xl sm:p-6" initial={{ opacity: 0, y: 18, scale: .98 }} animate={{ opacity: 1, y: 0, scale: 1 }} exit={{ opacity: 0, y: 12, scale: .98 }} onMouseDown={(event) => event.stopPropagation()}>
            ...
            <header className="mx-auto max-w-md text-center">
              <p className="text-xs font-semibold uppercase tracking-[.18em] text-[#ccd5ea]">Blueberry</p>
            ...
            <div className="mx-auto mt-4 flex max-w-sm rounded-xl bg-white/5 p-1">
              {(["focus", "break"] as TimerKind[]).map((option) => <button key={option} type="button" onClick={() => selectKind(option)} className={cn("min-h-10 flex-1 rounded-lg text-sm font-semibold capitalize transition", kind === option ? "bg-[#9989e2] text-slate-950" : "text-stone-300 hover:bg-white/10 hover:text-white")}>{option}</button>)}
            </div>
            ...
            <div className="group relative mx-auto mt-3 grid size-[min(60vw,230px)] place-items-center" tabIndex={0} aria-label="Timer adjustment controls">
              {adjustments.map((delta, index) => {
                ...
                return <button key={delta} type="button" disabled={isBlocked} onClick={() => adjust(delta)} style={{ transform: `translate(calc(-50% + ${x}px), calc(-50% + ${y}px))` }} className={cn("absolute left-1/2 top-1/2 z-10 grid size-10 place-items-center rounded-full border text-[11px] font-bold shadow-lg opacity-0 pointer-events-none transition duration-150 group-hover:pointer-events-auto group-hover:opacity-100 group-focus-within:pointer-events-auto group-focus-within:opacity-100 sm:size-11", delta > 0 ? "border-[#ccd5ea]/30 bg-[#9989e2] text-slate-950 hover:scale-105" : "border-white/15 bg-white/10 text-white hover:bg-white/20", isBlocked && "cursor-not-allowed opacity-30 hover:scale-100")}>{adjustmentLabel(delta)}</button>;
              })}
              <div className="grid size-28 place-items-center rounded-full border-[7px] border-[#bbb1eb]/70 bg-[#211b3a] text-center shadow-[0_0_0_15px_rgba(255,255,255,.04)] sm:size-32">
                <div><p className="font-mono text-3xl font-bold tabular-nums sm:text-4xl">{formatTime(remaining)}</p><p className="mt-1 text-xs font-medium text-[#e5eaf5]">{running ? "Running" : "Hover to adjust"}</p></div>
              </div>
            </div>
            ...
            <div className="mx-auto mt-4 flex max-w-md gap-2">
              <button type="button" onClick={() => { setRunning((value) => !value); setBannerDismissed(false); }} className="inline-flex min-h-11 flex-1 items-center justify-center gap-2 rounded-xl bg-[#9989e2] px-4 text-sm font-bold text-slate-950 transition hover:bg-[#bbb1eb]">{running ? <Pause className="size-4" /> : <Play className="size-4" />}{running ? "Pause" : "Start"}</button>
              <button type="button" onClick={reset} className="inline-flex min-h-11 items-center justify-center gap-2 rounded-xl border-2 border-[#ccd5ea]/50 px-4 text-sm font-bold text-white transition hover:border-[#ccd5ea] hover:bg-white/10"><RotateCcw className="size-4" />Reset</button>
            </div>

            <div className="mt-4 rounded-2xl border border-white/10 bg-white/[.045] p-4 sm:p-5">
              <div className="flex items-center justify-between gap-3"><div><h3 className="inline-flex items-center gap-2 text-sm font-bold"><ListTodo className="size-4 text-[#ccd5ea]" />Session tasks</h3><p className="mt-1 text-xs text-stone-300">Add notes, decks, or mind maps. They stay here for your next session.</p></div><span className="text-xs tabular-nums text-stone-300">{completedTasks}/{tasks.length}</span></div>
              ...
              <div className="mt-2 flex gap-2">{taskKind === "deck" ? <select value={selectedDeck} onChange={(event) => setSelectedDeck(event.target.value)} className="min-h-10 min-w-0 flex-1 rounded-lg border border-white/15 bg-[#211b3a] px-3 text-sm text-white outline-none focus:border-[#bbb1eb]"><option value="">Choose a study deck</option>{decks.map((deck) => <option key={deck.id} value={deck.id}>{deck.title}</option>)}</select> : <input value={draft} onChange={(event) => setDraft(event.target.value)} onKeyDown={(event) => { if (event.key === "Enter") addTask(); }} placeholder={taskKind === "mindmap" ? "Optional mind map label" : "Add a task"} className="min-h-10 min-w-0 flex-1 rounded-lg border border-white/15 bg-[#211b3a] px-3 text-sm text-white placeholder:text-stone-500 outline-none focus:border-[#bbb1eb]" />}
                <button type="button" onClick={addTask} className="inline-flex min-h-10 items-center gap-1 rounded-lg border border-[#ccd5ea]/30 bg-[#9989e2] px-3 text-sm font-bold text-slate-950 hover:bg-[#bbb1eb]"><Plus className="size-4" />Add</button></div>
              <ul className="mt-3 max-h-32 space-y-2 overflow-y-auto pr-1">{tasks.length === 0 ? <li className="rounded-lg border border-dashed border-white/10 px-3 py-3 text-xs text-stone-400">No tasks yet. Add one to make this session concrete.</li> : tasks.map((task) => <li key={task.id} className="flex min-h-10 items-center gap-2 rounded-lg bg-black/15 px-2.5"><button type="button" aria-label={`Mark ${task.label} ${task.done ? "incomplete" : "complete"}`} onClick={() => setTasks((current) => current.map((candidate) => candidate.id === task.id ? { ...candidate, done: !candidate.done } : candidate))} className={cn("grid size-5 shrink-0 place-items-center rounded border", task.done ? "border-[#bbb1eb] bg-[#bbb1eb] text-slate-950" : "border-white/35 text-transparent hover:border-[#ccd5ea]")}><Check className="size-3.5" /></button>{task.kind === "text" ? <span className={cn("min-w-0 flex-1 truncate text-sm", task.done ? "text-stone-500 line-through" : "text-white")}>{task.label}</span> : <button type="button" onClick={() => openLinkedTask(task)} className={cn("inline-flex min-w-0 flex-1 items-center gap-2 truncate text-left text-sm underline decoration-[#bbb1eb]/60 underline-offset-4", task.done ? "text-stone-500 line-through" : "text-[#e5eaf5] hover:text-white")}>{task.kind === "deck" ? <BookOpen className="size-4 shrink-0" /> : <Map className="size-4 shrink-0" />}<span className="truncate">{task.label}</span></button>}<button type="button" aria-label={`Remove ${task.label}`} onClick={() => setTasks((current) => current.filter((candidate) => candidate.id !== task.id))} className="grid size-7 shrink-0 place-items-center rounded text-stone-400 hover:bg-white/10 hover:text-white"><X className="size-4" /></button></li>)}</ul>
            </div>
      ...
      <AnimatePresence>
        {shouldShowBanner && <DockSlot order={DOCK.banner}><motion.aside role="status" aria-live="polite" className="flex w-[min(92vw,360px)] items-center gap-3 rounded-2xl border border-[#ccd5ea]/25 bg-[#171327]/95 p-3.5 text-white shadow-2xl backdrop-blur" initial={{ opacity: 0, y: 18 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: 18 }}><div className="grid size-9 shrink-0 place-items-center rounded-xl bg-[#9989e2] text-slate-950"><Clock3 className="size-4" /></div><button type="button" onClick={() => setDialogOpen(true)} className="min-w-0 flex-1 text-left"><p className="truncate text-xs font-semibold">{title}</p><p className="mt-0.5 font-mono text-lg font-bold tabular-nums">{formatTime(remaining)}</p></button><button type="button" onClick={() => setBannerDismissed(true)} aria-label="Dismiss timer banner" className="grid size-8 place-items-center rounded-lg text-stone-300 hover:bg-white/10 hover:text-white"><X className="size-4" /></button></motion.aside></DockSlot>}
      </AnimatePresence>
    </>, document.body)}
  </>;
}