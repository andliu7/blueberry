...
return <main className="relative min-h-screen text-slate-900 dark:text-stone-100"><PageBackground /><div className="mx-auto max-w-7xl px-4 py-5 sm:px-6 lg:px-8"><a href="#/home" className="inline-flex min-h-11 items-center gap-2 text-sm font-semibold text-slate-600 hover:text-[#472ab4] dark:text-stone-300"><ChevronLeft className="size-4"/>Home</a><header className="mt-5 grid gap-5 rounded-3xl border border-[#ccd5ea]/80 bg-white/95 p-6 shadow-sm backdrop-blur-sm dark:border-[#9989e2]/20 dark:bg-stone-950/92 lg:grid-cols-[1fr_270px]"><div><p className="font-mono text-xs font-semibold uppercase tracking-[.18em] text-[#5a3fd8] dark:text-[#bbb1eb]">Lessons / Organic chemistry II</p><h1 className="title-face mt-3 text-5xl leading-none">CHEM241</h1><p className="mt-4 max-w-2xl leading-7 text-slate-700 dark:text-stone-200">The ideas behind the cards, in the order the course takes them. Open a section for the writing, or expand it for every reaction underneath with its reagents, conditions and product.</p></div>{/* The berry reads along with you. `reading` looks down at the page rather
    than out at the room, which is the difference between a mascot that is
    keeping you company and one that is watching you. */}
<div className="flex flex-col items-center justify-center rounded-2xl border border-dashed border-[#bbb1eb] bg-[#f1f4fa]/70 p-4 dark:border-[#9989e2]/30 dark:bg-[#1f1456]/25"><Blueberry mood="reading" interactive onActivate={()=>{window.location.hash="#/home";}} className="h-40 w-40" label="Blueberry, reading along. Press to go home, drag to spin."/><p className="mt-2 text-center text-sm text-[#33208c]/75 dark:text-[#e5eaf5]/75">{topics.length} sections, {REACTIONS.length} checked reactions. Start with the overview if the carbonyl still looks like one reaction.</p></div></header>{/* Sidebar and content, rather than a row of pills over a single column.
...
{t.examples.length>0&&<section className="mt-5"><p className="font-mono text-xs font-semibold uppercase tracking-[.16em] text-slate-600 dark:text-stone-300">Worked examples</p><h2 className="title-face mt-1 text-2xl">Products and conditions</h2><div className="mt-3 grid gap-4 md:grid-cols-2 xl:grid-cols-3">{t.examples.map(([a,b,c,d])=><article key={a} className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm dark:border-stone-800 dark:bg-stone-950"><div className="flex h-36 items-center justify-center bg-gradient-to-br from-[#f1f4fa] via-white to-brand-to/10 p-3 dark:from-[#1f1456]/35 dark:via-stone-950 dark:to-brand-from/20"><img src={`${import.meta.env.BASE_URL}cards/${d}`} alt={"Product: "+c} className="h-full w-full object-contain" loading="lazy"/></div><div className="p-4"><h3 className="text-sm font-semibold">{a}</h3><p className="mt-1 font-mono text-[.7rem] leading-5 text-[#472ab4] dark:text-[#bbb1eb]">{b}</p><p className="mt-3 text-sm text-slate-700 dark:text-stone-200"><b>Product:</b> {c}</p></div></article>)}</div></section>}</>}</div></div></div><SiteFooter/></main>}
